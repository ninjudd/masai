...
First of all, decide what backend you want to use. Install whatever is necessary and get that running. After that, you'll want to `use` whatever backend you're using and Masai's protocol namespace, `flatland.masai.db`. For example, if you decided to use the redis backend:

    (ns my.ns
      (:use flatland.masai.db flatland.masai.redis))
